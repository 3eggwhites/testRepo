...
public class SimpleThreadpool {
	...
	/**
	 * Thrown when there's a RuntimeException or InterruptedException when
	 * executing a runnable from the queue, or awaiting termination
	 */
	...
	/**
	 * Inner Thread class which represents the threads in the pool. It acts as a
	 * wrapper for all runnables in the queue.
	 */
	private class SimpleThreadpoolThread extends Thread {
		...
		public void run() {
			try {
				// Continue to execute when the execute flag is true, or when
				// there are runnables in the queue
				while (execute.get() || !runnables.isEmpty()) {
					...
					// Sleep in case there wasn't any runnable in the queue.
					// This helps to avoid hogging the CPU.
					Thread.sleep(1);
				}
	...
	/**
	 * Private constructor to control the creation of threadpools. Increments
	 * the poolcount whenever a new pool is created.
	 *
	 * @param threadCount
	 *            Number of SimpleThreadpoolThreads to add to the pool
	 */
